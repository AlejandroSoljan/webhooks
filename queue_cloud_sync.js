...
function mountQueueSyncEndpoint(app, { getDb, secret = process.env.QUEUE_SYNC_SECRET, tenantList = process.env.QUEUE_SYNC_TENANTS || 'MCN' } = {}) {
  ...
  app.post('/api/ext/queue-sequence', express.json({ limit: '8kb' }), async (req, res) => {
    ...
    try {
      ...
      const counter = await counters.findOneAndUpdate({ _id: id }, { $inc: { sequence: 1 } }, { returnDocument: 'after' });
      return res.json({ ok: true, sequence: counter.sequence });
    } catch (error) { console.error('[QUEUE_SEQUENCE_API] error:', error.message); return res.status(500).json({ ok: false, error: 'sequence_failed' }); }
  });
