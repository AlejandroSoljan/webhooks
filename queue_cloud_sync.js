...
function mountQueueSyncEndpoint(app, { getDb, secret = process.env.QUEUE_SYNC_SECRET, tenantList = process.env.QUEUE_SYNC_TENANTS || 'MCN' } = {}) {
  ...
  app.post('/api/ext/queue-sync', express.json({ limit: '10mb' }), async (req, res) => {
    ...
    if (rows.length > 5000) return res.status(413).json({ ok: false, error: 'too_many_tickets' });
    try {
      ...
      return res.json({ ok: true, merged, tickets, serverTime: new Date().toISOString() });
    } catch (error) {
    ...
    try {
      ...
      for (const tenantId of tenants) {
        ...
        const response = await fetch(url, {
          ...
          body: JSON.stringify({ tenantId, since: initial ? null : since?.toISOString(), tickets }),
...
module.exports = { preferred, reviveTicket, mergeInto, mountQueueSyncEndpoint, startQueueCloudSync };
