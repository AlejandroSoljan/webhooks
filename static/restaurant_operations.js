...
(() => {
  ...
  async function action(name, payload = {}) {
    ...
    try { await api('/api/resto/operations/' + x.id, { action:name, revision:detailRevision, ...payload }); detailDirty = false; editingOrder = ''; await load(true); message('Cambios guardados.'); }
  ...
  root.addEventListener('click', async e => {
    ...
    if (button.id === 'opsClose' && confirm('¿Cerrar la cuenta y liberar esta mesa?')) await action('close');
