...
(() => {
  ...
  async function init() {
    try {
      ...
      reflectTenant();
      await load();
      select.onchange = () => { tenant = select.value; firstLoad = true; seen = new Set(); tableNotificationDrafts.clear(); el('events').textContent = 'Cargando…'; el('tables').textContent = 'Cargando…'; reflectTenant(); load(); };
