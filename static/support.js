...
(() => {
  ...
  let selected = null, saving = Promise.resolve(), saveTimer, rows = [], dirty = false, loading = false, renderedList = null, savingNow = false, ready = false;
  ...
  function show(row) {
    ...
    selectionLabel();
    $('memoryJid').value = row.jid; $('existingTickets').textContent = '';
  ...
  async function save() {
    ...
    saving = saving.catch(() => {}).then(async () => {
      ...
      try {
        ...
        selected.revision = result.revision; selected.fields = fields; selected.state = result.state || 'pending'; selectionLabel();
  ...
  $('memoryForm').onsubmit = action(async () => { await api('/memory', 'PUT', { jid: $('memoryJid').value, companyId: $('memoryCompany').value, contactId: $('memoryContact').value }); $('notice').textContent = 'Relación verificada y guardada.'; });
  ...
  async function initialize() {
    ...
    await Promise.all([session(), refresh()]);
  }
