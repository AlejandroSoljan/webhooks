...
test('a later same-topic conversation remains one draft after configured inactivity', async () => {
  ...
  assert.equal(drafts[0].messageIds.length, 2);
});
test('a later operator message creates another draft when it starts a different task', async () => {
  await processMessages([message('accounting', 0, { text: 'Necesito configurar bancos y el cierre contable' })]);
  await service.ingest(scope, message('access', 600, { fromMe: true, text: 'Ya tengo listo para instalar el acceso al sistema' }));
  now = new Date(+now + 180001); await service.runOne();
  const drafts = await service.listDrafts(scope);
