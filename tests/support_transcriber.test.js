...
test('support reuses the tenant transcription key and the existing Asisto usage path', async () => {
  let call;
  const provider = asistoTranscriber({ OPENAI_API_KEY: 'global-fixture' }, {
  ...
  assert.equal(call.openaiApiKey, 'tenant-fixture');
...
test('existing global key is reused; a missing key or empty transcription is not accepted as success', async () => {
  const options = { runtimeFor: async () => null, download: async () => Buffer.from('audio'), transcribe: async input => { assert.equal(input.openaiApiKey, 'existing'); return { text: '' }; } };
  const scope = { tenantId: 'a', userId: 'u' };
  await assert.rejects(() => asistoTranscriber({ OPENAI_API_KEY: 'existing' }, options).run('', {}, scope), /transcription_failed/);
