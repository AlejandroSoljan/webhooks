...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}html,body{margin:0;padding:0;background:transparent;color:var(--text);font-family:ui-sans-serif,system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif}
    ...
    .cards{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin-bottom:14px}.card{background:var(--panel);border:1px solid var(--border);border-radius:var(--radius);padding:16px;box-shadow:var(--shadow)}.metric{font-size:28px;font-weight:800;line-height:1}.label{font-size:12px;color:var(--muted);margin-top:6px}
    ...
    @media (max-width:1100px){.cards{grid-template-columns:repeat(2,minmax(0,1fr))}.layout{grid-template-columns:1fr}}@media (max-width:640px){.cards{grid-template-columns:1fr}}
  </style>
</head>
<body>
  <div class="page">
    <div class="head"><div><h1>Sesiones Telegram</h1><div class="sub">Estado de bots, chats y acciones por tenant</div></div><div class="actions"><button class="btn" id="tgReloadBtn" type="button">Recargar</button><button class="btn" id="tgRefreshBtn" type="button">Actualizar</button></div></div>
    <div id="tgMsg"></div>
    <div class="cards"><div class="card"><div class="metric" id="tgMetricTotal">0</div><div class="label">Bots configurados</div></div><div class="card"><div class="metric" id="tgMetricStarted">0</div><div class="label">Bots iniciados</div></div><div class="card"><div class="metric" id="tgMetricChats">0</div><div class="label">Chats conocidos</div></div><div class="card"><div class="metric" id="tgMetricBlocked">0</div><div class="label">Chats bloqueados</div></div></div>
  ...
  <script>
    (function(){
      ...
      function render(){ const q=String(searchEl.value||'').trim().toLowerCase(), filter=String(stateEl.value||'').trim(); const items=(state.items||[]).filter((item)=>rowMatches(item,q,filter)); const chats=(state.chats||[]).filter((chat)=>{ const hay=[chat.tenantId,chat.numero,chat.chatId,chat.username,chat.firstName,chat.lastName,chat.title,chat.chatType].join(' ').toLowerCase(); return !q || hay.includes(q); }); totalEl.textContent=String(state.items.length||0); startedEl.textContent=String((state.items||[]).filter((x)=>x.botStarted).length||0); chatsEl.textContent=String((state.chats||[]).length||0); blockedEl.textContent=String((state.chats||[]).filter((x)=>x.blocked).length||0); metaEl.textContent=items.length?('Mostrando '+items.length+' sesión'+(items.length===1?'':'es')+' · actualizado '+new Date().toLocaleString('es-AR')):'Sin sesiones visibles.'; bodyEl.innerHTML=!items.length?'<tr><td colspan="6" class="empty">No hay sesiones Telegram visibles.</td></tr>':items.map((item)=>{ const botLabel=item.telegramBotUsername?('@'+String(item.telegramBotUsername).replace(/^@+/,'')):'Sin username'; const lastSeen=item.lastSeenAt?new Date(item.lastSeenAt).toLocaleString('es-AR'):'-'; return '<tr>'+'<td><div class="tenantName">'+esc(item.tenantId)+'</div><div class="small">'+esc(item.numero||'-')+'</div></td>'+'<td><div>'+esc(botLabel)+'</div><div class="small"><code>'+esc(item.telegramBotId||'-')+'</code></div></td>'+'<td>'+statusPill(item)+'</td>'+'<td>'+esc(String(item.knownChats||0))+'</td>'+'<td>'+esc(lastSeen)+'</td>'+'<td><div class="rowActions"><button class="btn" type="button" data-action="start" data-tenant="'+esc(item.tenantId)+'">Iniciar</button><button class="btn btnDanger" type="button" data-action="release" data-tenant="'+esc(item.tenantId)+'">Detener</button></div></td>'+'</tr>'; }).join(''); chatsWrap.innerHTML=!chats.length?'<div class="empty">No hay chats registrados.</div>':'<table><thead><tr><th>Tenant</th><th>Chat</th><th>Usuario</th><th>Tipo</th><th>Última vez</th></tr></thead><tbody>'+chats.slice(0,200).map((chat)=>{ const displayName=[chat.firstName,chat.lastName].filter(Boolean).join(' ').trim()||chat.title||chat.username||'-'; return '<tr>'+'<td><div class="tenantName">'+esc(chat.tenantId)+'</div><div class="small">'+esc(chat.numero||'-')+'</div></td>'+'<td><div><code>'+esc(chat.chatId||'-')+'</code></div>'+(chat.blocked?'<div class="small" style="color:#b42318">Bloqueado</div>':'')+'</td>'+'<td><div>'+esc(displayName)+'</div><div class="small">'+esc(chat.username?('@'+String(chat.username).replace(/^@+/,'')):'-')+'</div></td>'+'<td>'+esc(chat.chatType||'-')+'</td>'+'<td>'+esc(chat.lastSeenAt?new Date(chat.lastSeenAt).toLocaleString('es-AR'):'-')+'</td>'+'</tr>'; }).join('')+'</tbody></table>'; }
