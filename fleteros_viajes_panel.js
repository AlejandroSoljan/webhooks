  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    @media (min-width:680px){
      .wrap{padding-top:18px}.row{grid-template-columns:1fr 1fr}.top h1{font-size:24px}.card{padding:18px}.grid2{grid-template-columns:repeat(4,1fr)}
    }
  </style>
</head>
<body>
  ...
  <main class="wrap">
    ...
    <div class="actions">
      ...
      <button id="btnSeed" class="btn2" style="width:100%;margin-top:8px" type="button">Cargar datos demo</button>
  ...
  <script>
  (function(){
    const state = { cliente:null, origen:null, destino:null, tipoCarga:null, chasis:null };
    ...
    function renderResults(type, items){
      ...
      if (!items || !items.length) {
        ...
        box.innerHTML = '<button type="button" class="item"><strong>Sin resultados</strong><span>Probá con otro texto o cargá datos demo.</span></button>';
        return;
      }
      box.className = 'results show';
    ...
    function bindAutocomplete(type){
      ...
      input.addEventListener('focus', function(){ search(type, input.value).catch(function(){}); });
    }
    ...
    async function seedDemo(){
      ...
      toast('Datos demo cargados para este dominio', true);
      ['clientes','origen','destino','tipos-carga','chasis'].forEach(function(type){ const input = el(type + 'Input'); if (input) search(type, input.value).catch(function(){}); });
    }
    ...
    async function submit(){
      ...
      try {
        ...
        const r = await fetch('/api/fleteros/viajes', {
          method:'POST',
        ...
        toast('Viaje registrado correctamente', true);
        const box = el('lastTrip');
        box.className = 'lastTrip show';
        box.innerHTML = '<b>Último viaje:</b><br>' +
        ...
        toast('No se pudo registrar el viaje: ' + (e.message || 'error'), false);
      } finally {
  ...
  app.get('/api/fleteros/viajes', requireAuth, async (req, res) => {
    try {
      ...
      res.json({
        ok: true,
        tenantId,
        items: rows.map((v) => ({ id: String(v._id), ...v, _id: String(v._id) })),
      });
    } catch (e) {
