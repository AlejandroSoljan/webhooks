...
// - Clasificación automática con IA durante la conversación y clasificación final al cerrar.
...
const LIVE_AI_PER_SWEEP = 4;
const LIVE_AI_QUIET_MS = 15 * 1000;
...
function queryList(value, allowed = []) {
  const allow = new Set((allowed || []).map(v => String(v).toLowerCase()));
  const raw = Array.isArray(value) ? value : String(value ?? '').split(',');
  return [...new Set(raw
    .map(v => cleanString(v, 50).toLowerCase())
    .filter(v => v && (!allow.size || allow.has(v))))];
}

function effectiveWorkflow(conv, followup) {
  if (conv?.finalized !== true) return conv?.manualOpen === true ? 'manual' : 'active';
...
function publicConversation(conv, followup, lead, config) {
  ...
  return {
    ...
    contactPhone: conv?.contactPhone || lead?.phone || '',
    contactEmail: conv?.contactEmail || lead?.email || '',
    channelType: conv?.channelType || 'whatsapp',
    displayPhoneNumber: conv?.displayPhoneNumber || '',
    qrProductCode: conv?.qrProductCode || '',
    qrProductDescription: conv?.qrProductDescription || '',
    status: conv?.status || (conv?.finalized ? 'CLOSED' : 'OPEN'),
    finalized: conv?.finalized === true,
    manualOpen: conv?.manualOpen === true,
    manualOperator: conv?.manualOperator || '',
    manualStartedAt: conv?.manualStartedAt || null,
    ...
    aiClassificationStage: followup?.aiClassificationStage || '',
    aiProvisional: followup?.aiClassificationStage === 'live',
...
async function classifyConversationWithAi(db, tenant, conversationId, { force = false, live = false } = {}) {
  ...
  try {
    ...
    const isFinal = conv.finalized === true;
    if (!isFinal && !live && !force) return { ok: true, skipped: 'conversation_open' };
    if (!force && hasManualClassification(existing)) return { ok: true, skipped: 'manual_classification' };

    if (!force) {
      if (isFinal) {
        if (existing?.aiClassificationStatus === 'done' && existing?.aiClassificationStage === 'final') {
          return { ok: true, skipped: 'already_classified_final' };
        }
      } else {
        const convLastAt = safeDate(lastActivity(conv));
        const aiAt = safeDate(existing?.aiClassifiedAt);
        if (convLastAt && (Date.now() - convLastAt.getTime()) < LIVE_AI_QUIET_MS) {
          return { ok: true, skipped: 'live_waiting_for_quiet' };
        }
        if (aiAt && (Date.now() - aiAt.getTime()) < LIVE_AI_MIN_INTERVAL_MS) {
          return { ok: true, skipped: 'live_throttled' };
        }
        if (aiAt && convLastAt && convLastAt.getTime() <= aiAt.getTime()) {
          return { ok: true, skipped: 'live_unchanged' };
        }
      }
    }
    ...
    const system = [
      ...
      isFinal
        ? 'La conversación está FINALIZADA. Generá la clasificación definitiva.'
        : 'La conversación sigue ABIERTA. Generá una clasificación PROVISORIA usando sólo la evidencia disponible hasta ahora; puede cambiar más adelante.',
      ...
      isFinal
        ...
        : 'satisfaction es PROVISORIA mientras la conversación siga abierta: 5 muy satisfecho, 4 satisfecho, 3 neutral o sin evidencia clara, 2 insatisfecho, 1 muy insatisfecho.',
    ...
    const preserveManualSummary = !!existing?.manualEditedAt && !!cleanString(existing?.summary, 1000);
    const set = {
      category: normalizeCategory(parsed.category),
      satisfaction: normalizeSatisfaction(parsed.satisfaction) || 3,
      quoteRequested: quoteDetected ? true : !!parsed.quote_requested,
      summary: cleanString(preserveManualSummary ? existing.summary : parsed.summary, 1000),
      ...
      aiClassificationStage: isFinal ? 'final' : 'live',
      ...
      updatedAt: now,
      updatedBy: force ? 'asisto-ai-reclassify' : (isFinal ? 'asisto-ai' : 'asisto-ai-live'),
    };
    await db.collection(FOLLOWUP_COLLECTION).updateOne(
      { tenantId: tenant, conversationId: oid },
    ...
    await db.collection('conversations').updateOne(
      ...
      { $set: {
        followupReviewPending: isFinal ? true : !!conv.followupReviewPending,
        followupWorkflowStatus: existing?.workflowStatus || (isFinal ? 'pending_review' : (conv.followupWorkflowStatus || '')),
        followupAiClassifiedAt: now,
        followupAiClassificationStage: isFinal ? 'final' : 'live',
      } }
    );
    await recordClassificationUsage(db, tenant, conv, response?.model || runtime.model, response?.usage || {});
    console.log(`[followup][ai] ${isFinal ? 'final' : 'provisoria'} tenant=${tenant} conv=${String(oid)} tipo=${set.category} satisfaccion=${set.satisfaction} cotizacion=${set.quoteRequested}`);
...
async function classifyActiveConversations(db, tenant, limit = LIVE_AI_PER_SWEEP) {
  const convs = await db.collection('conversations')
    .find({ tenantId: tenant, botMode: 'conversacional', finalized: { $ne: true } })
  ...
  const followups = await db.collection(FOLLOWUP_COLLECTION)
    .find({ tenantId: tenant, conversationId: { $in: ids } })
    .toArray();
  ...
  const pending = convs.filter(c => {
    ...
    const convLastAt = safeDate(lastActivity(c));
    if (!convLastAt) return false;
    if ((nowMs - convLastAt.getTime()) < LIVE_AI_QUIET_MS) return false;

    const aiAt = safeDate(f?.aiClassifiedAt);
    if (aiAt && (nowMs - aiAt.getTime()) < LIVE_AI_MIN_INTERVAL_MS) return false;
    if (aiAt && convLastAt.getTime() <= aiAt.getTime()) return false;

    const retryAt = safeDate(f?.aiClassificationNextRetryAt);
    if (!shouldBypassAiRetry(f) && retryAt && retryAt.getTime() > nowMs) return false;
  ...
  for (const conv of pending) {
    const r = await classifyConversationWithAi(db, tenant, conv._id, { force: false, live: true });
...
async function classifyMissingFinalized(db, tenant, limit = AI_BACKFILL_PER_SWEEP) {
  ...
  const pending = convs.filter(c => {
    const f = fm.get(String(c._id));
    if (hasManualClassification(f)) return false;
    if (f?.aiClassifiedAt && f?.aiClassificationStatus === 'done' && f?.aiClassificationStage === 'final') return false;
    const retryAt = safeDate(f?.aiClassificationNextRetryAt);
...
async function autoCloseTenant(db, tenant, config) {
  ...
  const result = await db.collection('conversations').updateMany(
    { tenantId: tenant, botMode: 'conversacional', finalized: { $ne: true }, manualOpen: { $ne: true }, updatedAt: { $lte: cutoff } },
...
async function runAutoCloseSweep() {
  ...
  for (const raw of tenants) {
    ...
    try {
      ...
      await classifyActiveConversations(db, tenant, LIVE_AI_PER_SWEEP);
    } catch (e) {
...
@media(max-width:1100px){.filters{grid-template-columns:1fr 1fr 1fr}.kpis{grid-template-columns:repeat(3,1fr)}.aiGrid{grid-template-columns:1fr 1fr}table{min-width:980px}.actionsCell .btnSm{padding:4px 6px;font-size:10px}}@media(max-width:700px){.wrap{padding:8px}.filters{grid-template-columns:1fr 1fr}.filters>input{grid-column:1/-1}.kpis{grid-template-columns:repeat(3,1fr)}.grid2{grid-template-columns:1fr}.aiGrid{grid-template-columns:1fr 1fr}.modal{padding:6px}.modalCard{max-height:97vh}}@media(max-width:460px){.filters{grid-template-columns:1fr}.filters>input{grid-column:auto}.kpis{grid-template-columns:1fr 1fr}}
 </style>
</head>
<body>
<div class="wrap">
  ...
  <div class="kpis">
    <div class="kpi"><b id="kActive">0</b><span>Bot activo</span></div>
    <div class="kpi"><b id="kManual">0</b><span>Atención manual</span></div>
  ...
  <div class="filters">
    ...
    <div class="dateFilter"><label for="dateFrom">Desde</label><input id="dateFrom" type="date"/></div>
    <div class="dateFilter"><label for="dateTo">Hasta</label><input id="dateTo" type="date"/></div>
    ...
    <details class="multiFilter" id="stateFilter">
      <summary>Estado: Todos</summary>
      <div class="multiMenu">
        <div class="multiTools"><button type="button" data-select-all="state">Todos</button><button type="button" data-clear-all="state">Ninguno</button></div>
        <label><input type="checkbox" name="state" value="open" checked/>Abiertas</label>
        <label><input type="checkbox" name="state" value="manual" checked/>Atención manual</label>
        <label><input type="checkbox" name="state" value="closed" checked/>Finalizadas</label>
      </div>
    </details>

    <details class="multiFilter" id="workflowFilter">
      <summary>Gestión: Todas</summary>
      <div class="multiMenu">
        <div class="multiTools"><button type="button" data-select-all="workflowFilterValue">Todas</button><button type="button" data-clear-all="workflowFilterValue">Ninguna</button></div>
       <label><input type="checkbox" name="workflowFilterValue" value="pending_review" checked/>Pendiente</label>
        <label><input type="checkbox" name="workflowFilterValue" value="follow_up" checked/>En seguimiento</label>
        <label><input type="checkbox" name="workflowFilterValue" value="due" checked/>Contacto vencido</label>
        <label><input type="checkbox" name="workflowFilterValue" value="resolved" checked/>Resuelta</label>
        <label><input type="checkbox" name="workflowFilterValue" value="discarded" checked/>Descartada</label>
      </div>
    </details>

    <details class="multiFilter" id="categoryFilter">
      <summary>Tipo: Todos</summary>
      <div class="multiMenu">
        <div class="multiTools"><button type="button" data-select-all="categoryFilterValue">Todos</button><button type="button" data-clear-all="categoryFilterValue">Ninguno</button></div>
        <label><input type="checkbox" name="categoryFilterValue" value="consulta" checked/>Consulta</label>
        <label><input type="checkbox" name="categoryFilterValue" value="cotizacion" checked/>Cotización</label>
        <label><input type="checkbox" name="categoryFilterValue" value="soporte" checked/>Soporte</label>
        <label><input type="checkbox" name="categoryFilterValue" value="reclamo" checked/>Reclamo</label>
        <label><input type="checkbox" name="categoryFilterValue" value="comercial" checked/>Comercial</label>
        <label><input type="checkbox" name="categoryFilterValue" value="otro" checked/>Otro</label>
        <label><input type="checkbox" name="categoryFilterValue" value="sin_clasificar" checked/>Sin clasificar</label>
      </div>
    </details>  </div>
...
<div class="modal" id="chatModal"><div class="modalCard wide"><div class="modalHead"><div><h3 id="chatTitle">Conversación</h3><div class="muted" id="chatSubtitle"></div><div class="contactLine" id="chatContact"></div></div><button class="btn btnSm" data-close="chatModal">Cerrar</button></div><div class="chat" id="chatBody"><div class="empty">Cargando…</div></div><div class="chatComposer hidden" id="chatComposer"><textarea id="chatMessage" maxlength="4000" placeholder="Escribí una respuesta al cliente…"></textarea><button class="btn btnPrimary" id="chatSendBtn" type="button">Enviar</button></div><div class="modalFoot" id="chatFoot"></div></div></div>
...
function closeModal(id){el(id).classList.remove('open');if(id==='chatModal'&&chatPollTimer){clearInterval(chatPollTimer);chatPollTimer=null}}
...
function allValues(name){return [...document.querySelectorAll('input[name="'+name+'"]')].map(x=>x.value)}
function updateMultiSummary(detailsId,name,prefix){const root=el(detailsId);if(!root)return;const selected=checkedValues(name);const all=allValues(name);let txt=prefix+': ';if(!selected.length)txt+='Ninguno';else if(selected.length===all.length)txt+='Todos';else txt+=selected.length+' seleccionados';root.querySelector('summary').textContent=txt}
function setupMulti(detailsId,name,prefix){const root=el(detailsId);if(!root)return;const trigger=()=>{updateMultiSummary(detailsId,name,prefix);clearTimeout(debounce);debounce=setTimeout(()=>loadRows().catch(e=>toast(e.message,true)),0)};root.querySelectorAll('input[name="'+name+'"]').forEach(x=>x.addEventListener('change',trigger));root.querySelectorAll('[data-select-all]').forEach(b=>b.addEventListener('click',e=>{e.preventDefault();root.querySelectorAll('input[name="'+name+'"]').forEach(x=>x.checked=true);trigger()}));root.querySelectorAll('[data-clear-all]').forEach(b=>b.addEventListener('click',e=>{e.preventDefault();root.querySelectorAll('input[name="'+name+'"]').forEach(x=>x.checked=false);trigger()}));updateMultiSummary(detailsId,name,prefix)}
function localDateBoundary(id,endOfDay){const v=String(el(id)?.value||'').trim();if(!v)return '';const d=new Date(v+'T00:00:00');if(isNaN(d))return '';if(endOfDay)d.setHours(23,59,59,999);return d.toISOString()}
...
function apiWindowSummaryText(x){
  const open=x.windowEndsAt&&new Date(x.windowEndsAt).getTime()>Date.now();
  return [
    'Último envío: '+fmt(x.lastMessageAt),
    'Ventana: '+String(x.windowMinutes||20)+' min',
    'Mensajes: '+String(x.messageCount||0),
    'Importe: '+fmtMoney(x.amount||0,x.currency||'ARS'),
    'Estado: '+(open?'Abierta':'Cerrada')
  ].join(' · ');
}
function renderRows(){
  const mixed=[
    ...rows.map(x=>({kind:'conversation',at:x.lastAt||x.updatedAt||x.createdAt||'',item:x})),
    ...apiWindows.map(x=>({kind:'api_messages',at:x.windowStartedAt||x.lastMessageAt||'',item:x}))
  ].sort((a,b)=>{
  ...
  el('listCount').textContent=mixed.length+' registros';
  if(!mixed.length){
    el('tbody').innerHTML='<tr><td colspan="10" class="empty">No hay registros con estos filtros.</td></tr>';
    return;
  }

  el('tbody').innerHTML=mixed.map(entry=>{
    if(entry.kind==='api_messages'){
      const x=entry.item||{};
      const recipient=x.contact||x.contactName||'Sin nombre';
      const sub=[
        'API Mensajes',
        (x.contactName&&x.contactName!==x.contact)?x.contactName:'',
        x.numeroFrom?('desde '+x.numeroFrom):''
      ].filter(Boolean).join(' · ');
      const summary=apiWindowSummaryText(x);
      return '<tr class="apiMessageRow">'+
        '<td><div class="clientName" title="'+esc(recipient)+'">'+esc(recipient)+'</div><div class="phone">'+esc(sub)+'</div></td>'+
        ...
        '<td>—</td>'+ 
        '<td>—</td>'+ 
        '<td><div class="summaryCell" title="'+esc(summary)+'">'+esc(summary)+'</div></td>'+ 
        '<td>—</td>'+ 
        '<td><div class="actionsCell"><button class="btn btnSm" data-api-window="'+esc(x._id)+'">Ver mensajes</button></div></td>'+ 
      '</tr>';
    }

    const x=entry.item;
    const nm=x.channelType==='qr_web'?(x.lead?.name||x.contactName||x.waId||'Sin nombre'):(x.contactName||x.lead?.name||x.waId||'Sin nombre');
    const qrContact=x.lead?.phone||x.contactPhone||x.lead?.email||x.contactEmail||'';
    const channelInfo=x.channelType==='qr_web'?('QR Web'+(x.qrProductCode?' · SKU '+x.qrProductCode:'')+(qrContact?' · '+qrContact:'')):(x.waId||'');
    const status=x.finalized?'<span class="badge bClosed">Finalizada</span>':(x.manualOpen?'<span class="badge bFollow">Atención manual</span>':'<span class="badge bOpen">Bot activo</span>');
    const workflow='<span class="badge '+workflowBadge(x.workflowStatus)+'">'+esc(workflowLabel(x.workflowStatus))+'</span>'+(x.isDue?' <span class="badge bDue">Vencido</span>':'');
    const cat=x.category?'<span class="badge bYes">'+esc(categoryLabel(x.category))+'</span> '+aiBadge(x):aiBadge(x)||'<span class="badge bNo">Sin clasificar</span>';
    const next=x.pendingContact?(x.isDue?'<span class="badge bDue">'+esc(fmt(x.nextContactAt))+'</span>':esc(fmt(x.nextContactAt))):'—';
    return '<tr><td><div class="clientName" title="'+esc(nm)+'">'+esc(nm)+'</div><div class="phone">'+esc(channelInfo)+'</div></td><td>'+esc(fmt(x.lastAt))+'</td><td>'+status+'</td><td>'+workflow+'</td><td>'+cat+'</td><td>'+quoteBadge(x.quoteRequested,x.finalized)+'</td><td>'+stars(x.satisfaction)+'</td><td><div class="summaryCell" title="'+esc(x.summary||'')+'">'+esc(x.summary||'—')+'</div></td><td>'+next+'</td><td><div class="actionsCell"><button class="btn btnSm" data-chat="'+esc(x._id)+'">💬 Ver chat</button><button class="btn btnSm btnPrimary" data-follow="'+esc(x._id)+'">✎ Seguimiento</button></div></td></tr>';
  }).join('');

  el('tbody').querySelectorAll('[data-chat]').forEach(b=>b.addEventListener('click',()=>openChat(b.dataset.chat)));
  el('tbody').querySelectorAll('[data-follow]').forEach(b=>b.addEventListener('click',()=>openFollow(b.dataset.follow)));
  el('tbody').querySelectorAll('[data-api-window]').forEach(b=>b.addEventListener('click',()=>openApiWindow(b.dataset.apiWindow)));
}
...
async function refreshOpenChatMessages(id){
  ...
  try{const m=await requestJson('/api/conversation-followup/'+encodeURIComponent(id)+'/messages');renderFollowupChatMessages(m.items||[])}catch(_){}
}
function startOpenChatPolling(id){if(chatPollTimer)clearInterval(chatPollTimer);chatPollTimer=setInterval(()=>refreshOpenChatMessages(id),3000)}
async function sendFromFollowupChat(){
...
async function openChat(id){
  ...
  }catch(e){el('chatBody').innerHTML='<div class="empty">Error cargando el chat.</div>';toast(e.message,true)}
}
...
async function closeNow(id){if(!confirm('¿Finalizar esta conversación ahora? Se hará una clasificación final con IA.'))return;try{toast('Finalizando y clasificando…');await requestJson('/api/conversation-followup/'+encodeURIComponent(id)+'/close',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});toast('Conversación finalizada');closeModal('chatModal');await loadRows();if(el('followModal').classList.contains('open'))await openFollow(id)}catch(e){toast(e.message,true)}}
...
el('chatSendBtn').addEventListener('click',sendFromFollowupChat);el('chatMessage').addEventListener('keydown',e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();sendFromFollowupChat()}});
el('refreshBtn').addEventListener('click',refresh);if(CAN_SELECT_TENANT&&el('tenantSelect'))el('tenantSelect').addEventListener('change',e=>changeTenant(e.target.value));el('q').addEventListener('input',()=>{clearTimeout(debounce);debounce=setTimeout(()=>loadRows().catch(e=>toast(e.message,true)),250)});['dateFrom','dateTo'].forEach(id=>el(id).addEventListener('change',()=>loadRows().catch(e=>toast(e.message,true))));setupMulti('channelFilter','channelFilterValue','Canal');setupMulti('stateFilter','state','Estado');setupMulti('workflowFilter','workflowFilterValue','Gestión');setupMulti('categoryFilter','categoryFilterValue','Tipo');
...
function mountConversationFollowupPanel(app, { auth } = {}) {
  ...
  app.get('/api/conversation-followup/conversations', async (req, res) => {
    try {
      ...
      setImmediate(() => classifyActiveConversations(db, tenant, LIVE_AI_PER_SWEEP).catch(e => console.warn('[followup][ai] live list:', e?.message || e)));

      const limit = intValue(req.query?.limit, 500, 1, 1000);
      const qText = cleanString(req.query?.q, 150);
      const rawChannels = cleanString(req.query?.channel || 'all', 120).toLowerCase();
      const rawStates = cleanString(req.query?.conversationState || 'all', 120).toLowerCase();
      const rawWorkflows = cleanString(req.query?.workflow || 'all', 180).toLowerCase();
      const rawCategories = cleanString(req.query?.category || 'all', 180).toLowerCase();
      const noChannels = rawChannels === 'none';
      const noStates = rawStates === 'none';
      const noWorkflows = rawWorkflows === 'none';
      const noCategories = rawCategories === 'none';
      const channels = (rawChannels === 'all' || noChannels) ? [] : queryList(rawChannels, ['whatsapp', 'qr_web']);
      const conversationStates = (rawStates === 'all' || noStates) ? [] : queryList(rawStates, ['open', 'manual', 'closed']);
      const workflows = (rawWorkflows === 'all' || noWorkflows) ? [] : queryList(rawWorkflows, ['pending_review', 'follow_up', 'due', 'resolved', 'discarded']);
      const categories = (rawCategories === 'all' || noCategories) ? [] : queryList(rawCategories, ['consulta', 'cotizacion', 'soporte', 'reclamo', 'comercial', 'otro', 'sin_clasificar']);
      ...
      const fetchLimit = (dateFrom || dateTo) ? 5000 : Math.min(2000, limit * 3);
      const convs = await db.collection('conversations').find(q).sort({ updatedAt: -1, openedAt: -1 }).limit(fetchLimit).toArray();
      const ids = convs.map(c => c._id);
      ...
      if (noChannels || noStates || noWorkflows || noCategories) {
         items = [];
      }

      if (channels.length) {
        items = items.filter(x => channels.includes(String(x.channelType || 'whatsapp').trim().toLowerCase() || 'whatsapp'));
      }
      ...
      if (dateFrom || dateTo) {
        ...
        items = items.filter(x => {
          const ms = x.lastAt ? new Date(x.lastAt).getTime() : NaN;
          return Number.isFinite(ms) && ms >= fromMs && ms <= toMs;
        });
      }

      if (conversationStates.length) {
        items = items.filter(x => conversationStates.some(state =>
          state === 'open' ? (!x.finalized && !x.manualOpen)
            : state === 'manual' ? (!x.finalized && x.manualOpen)
              : state === 'closed' ? x.finalized
                : false
        ));
      }

      if (workflows.length) {
        items = items.filter(x => workflows.some(value => value === 'due' ? x.isDue : x.workflowStatus === value));
      }

      if (categories.length) {
        items = items.filter(x => categories.some(value =>
          value === 'sin_clasificar' ? !String(x.category || '').trim() : String(x.category || '') === value
        ));
      }
      ...
      const summary = {
        active: allForSummary.filter(x => !x.finalized && !x.manualOpen).length,
        manual: allForSummary.filter(x => !x.finalized && x.manualOpen).length,
  ...
  app.get('/api/conversation-followup/:id', async (req, res) => {
    try {
      ...
      // temperature: 0. La versión actual NO envía ese parámetro.
      if (bundle.conv?.finalized === true && shouldBypassAiRetry(bundle.followup)) {
        ...
        await classifyConversationWithAi(db, tenant, bundle.conv._id, { force: true, live: false });
        bundle = await loadConversationBundle(db, tenant, id);
      } else if (bundle.conv?.finalized !== true) {
        const liveResult = await classifyConversationWithAi(db, tenant, bundle.conv._id, { force: false, live: true });
        if (liveResult?.ok && !liveResult?.skipped) bundle = await loadConversationBundle(db, tenant, id);
      }
  ...
  app.post('/api/conversation-followup/:id/send', async (req, res) => {
    try {
      if (!hasPageAccess(auth, req, 'inbox')) return res.status(403).json({ ok: false, error: 'forbidden' });
      const tenant = resolveTenant(req, auth);
      const id = String(req.params.id || '').trim();
      ...
      if (!conv) return res.status(404).json({ ok: false, error: 'not_found' });
      if (conv.finalized === true) return res.status(409).json({ ok: false, error: 'conversation_finalized' });
      if (String(conv.channelType || '').toLowerCase() !== 'qr_web') return res.status(400).json({ ok: false, error: 'qr_web_only' });
      ...
      await db.collection('messages').insertOne({
        ...
        meta: {
          ...
          manual: true,
          qrProductCode: conv.qrProductCode || '',
          requestedBy: req.user?.username || req.user?.uid || req.user?.email || 'operador',
        },
      });

      const set = {
        manualOpen: true,
        manualOperator: cleanString(req.user?.username || req.user?.uid || req.user?.email || 'operador', 120),
        lastAssistantTs: now,
        updatedAt: now,
        status: 'OPEN',
      };
      if (conv.manualOpen !== true) set.manualStartedAt = now;
      await db.collection('conversations').updateOne({ _id: conv._id, tenantId: tenant }, { $set: set });
      return res.json({ ok: true, manualOpen: true });
    } catch (e) {
      console.error('[followup] send qr message:', e);
      return res.status(500).json({ ok: false, error: 'internal' });
    }
  });
  ...
  app.post('/api/conversation-followup/:id/classify', async (req, res) => {
    try {
      ...
      const conv = await db.collection('conversations').findOne({ _id: new ObjectId(id), tenantId: tenant, botMode: 'conversacional' });
      if (!conv) return res.status(404).json({ ok: false, error: 'not_found' });
      const result = await classifyConversationWithAi(db, tenant, conv._id, { force: boolValue(req.body?.force, true), live: conv.finalized !== true });
  ...
  app.post('/api/conversation-followup/:id/close', async (req, res) => {
    try {
      ...
      const r = await db.collection('conversations').updateOne(
        ...
        { $set: { finalized: true, manualOpen: false, status: 'CLOSED_MANUAL', closedAt: now, updatedAt: now, closeReason: 'operator', manualEndedAt: now, followupReviewPending: true } }
      );
