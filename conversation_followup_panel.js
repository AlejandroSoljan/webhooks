...
const FOLLOWUP_BUILD = '2026-08-24-v5-filtros-multiples-fechas';
...
async function loadRows(){const p=new URLSearchParams();p.set('limit','500');const q=el('q').value.trim();if(q)p.set('q',q);const states=checkedValues('state');const workflows=checkedValues('workflowFilterValue');const categories=checkedValues('categoryFilterValue');const allStates=allValues('state');const allWorkflows=allValues('workflowFilterValue');const allCategories=allValues('categoryFilterValue');p.set('conversationState',states.length===allStates.length?'all':states.join(','));p.set('workflow',workflows.length===allWorkflows.length?'all':workflows.join(','));p.set('category',categories.length===allCategories.length?'all':categories.join(','));const dateFrom=localDateBoundary('dateFrom',false);const dateTo=localDateBoundary('dateTo',true);if(dateFrom)p.set('dateFrom',dateFrom);if(dateTo)p.set('dateTo',dateTo);const j=await requestJson('/api/conversation-followup/conversations?'+p.toString());rows=j.items||[];renderKpis(j.summary||{});renderRows()}
...
el('refreshBtn').addEventListener('click',refresh);if(CAN_SELECT_TENANT&&el('tenantSelect'))el('tenantSelect').addEventListener('change',e=>changeTenant(e.target.value));el('q').addEventListener('input',()=>{clearTimeout(debounce);debounce=setTimeout(()=>loadRows().catch(e=>toast(e.message,true)),250)});['dateFrom','dateTo'].forEach(id=>el(id).addEventListener('change',()=>loadRows().catch(e=>toast(e.message,true))));setupMulti('stateFilter','state','Estado');setupMulti('workflowFilter','workflowFilterValue','Gestión');setupMulti('categoryFilter','categoryFilterValue','Tipo');
...
function mountConversationFollowupPanel(app, { auth } = {}) {
  ...
  app.get('/api/conversation-followup/conversations', async (req, res) => {
    try {
      ...
      const conversationStates = rawStates === 'all' ? [] : queryList(rawStates, ['open', 'manual', 'closed']);
      const workflows = rawWorkflows === 'all' ? [] : queryList(rawWorkflows, ['pending_review', 'follow_up', 'due', 'resolved', 'discarded']);
      const categories = rawCategories === 'all' ? [] : queryList(rawCategories, ['consulta', 'cotizacion', 'soporte', 'reclamo', 'comercial', 'otro', 'sin_clasificar']);
