...
let port = Number(process.env.PORT || 8002);
let seg_desde = 8000;
let seg_hasta = 12000;
let seg_msg = 5000;
let seg_tele = 3000;
...
const logFilePath_event = path.join(__dirname, 'app_asisto_tg_event.log');
const logFilePath_error = path.join(__dirname, 'app_asisto_tg_error.log');

EscribirLog('inicio Script Telegram', 'event');
...
app.get('/', (req, res) => {
  res.json({ ok: true, script: 'app_asisto_tg', state: localBotState, tenantId, numero, instanceId });
});
...
async function ensureMongo() {
  try {
    ...
    if (globalThis.__asistoTgMongoConnectingPromise) {
      const ok = await globalThis.__asistoTgMongoConnectingPromise;
    ...
    globalThis.__asistoTgMongoConnectingPromise = (async () => {
      try {
        ...
        globalThis.__asistoTgMongoConnectingPromise = null;
      }
    })();
...
app.get('/status/qr', requireStatusToken, async (req, res) => {
  return res.json({
    ...
    botState: localBotState,
    notApplicable: true,
    reason: 'Telegram bot usa token, no QR.'
  });
});
...
async function procesar_mensaje(json, message) {
  ...
  for (let i = jsonGlobal[indice][1]; i < tam_json; i++) {
    ...
    if (i <= cant_lim + jsonGlobal[indice][1] - 1) {
      ...
      await io.emit('message', 'Respuesta: ' + chatId + ': ' + mensaje);
      if (tam_json - 1 === i) {
...
async function handleIncomingTelegramMessage(msg) {
  try {
    ...
    if (valor_i === 0) {
      ...
      await io.emit('message', 'Mensaje: ' + telefonoFrom + ': ' + body);
...
function attachBotHandlers() {
  ...
  bot.on('polling_error', async (err) => {
    ...
    await io.emit('message', 'Telegram polling_error');
  });
  ...
  bot.on('webhook_error', async (err) => {
    ...
    await io.emit('message', 'Telegram webhook_error');
  });
}
...
async function startBotInitialize() {
  ...
  try {
    ...
    await io.emit('message', 'Telegram listo!');
    ...
    for (let i = 0; i < destinatarios.length; i++) {
      ...
      for (let j = 0; j < respuestas.length; j++) {
        ...
        if (chatId === '' || chatId === null) {
          await io.emit('message', 'Mensaje: ' + nro_tel + ': Chat inválido');
        ...
        if (!chatData) {
          ...
          await io.emit('message', 'Mensaje: ' + String(chatId) + ': Chat no conocido por el bot');
          await actualizar_estado_mensaje(url_confirma_msg, 'I', null, null, null, null, null, idRenglon, idDest);
          continue;
        }
        ...
        try {
          ...
          await io.emit('message', 'Mensaje: ' + String(chatId) + ': ' + Msj);
...
(async function startAsistoTelegram() {
  try {
    RecuperarJsonConf();
    ...
    if (!tenantId || !mongo_uri) {
      throw new Error('Falta tenantId/mongo_uri en configuracion.json');
    }

    if (!telegram_bot_token) {
      throw new Error('Falta telegram_bot_token/bot_token en tenant_config o configuracion.json');
    }

    server.listen(port, function () {
      EscribirLog('App Telegram running on *: ' + port, 'event');
    });

    bootstrapWithLock().catch((e) => {
      EscribirLog('bootstrap inicio directo Telegram error: ' + String(e?.message || e), 'error');
    });
  } catch (e) {
    EscribirLog('FATAL bootstrap Telegram: ' + String(e?.message || e), 'error');
    try {
      server.listen(port, function () {
        EscribirLog('App Telegram running on *: ' + port, 'event');
      });
