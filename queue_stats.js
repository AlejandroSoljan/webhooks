...
function ticketVisits(doc) {
  ...
  if (['RESERVED', 'CANCELLED'].includes(doc.status) && !activation) return [];
  ...
  for (const h of history) {
    ...
    if (['finish', 'skip', 'transfer'].includes(h.action)) {
...
function summarize(rows, sectors) {
  ...
  const days = new Map();
  for (const d of rows) { if (!days.has(d.dayKey)) days.set(d.dayKey, { day: d.dayKey, issued: 0, finished: 0, expired: 0 }); const g = days.get(d.dayKey); g.issued++; g.finished += Number(d.status === 'DONE'); g.expired += Number(d.status === 'CANCELLED'); }
...
function statsPage(tenant, today) {
...
module.exports = { mountQueueStats, ticketVisits, summarize, statsPage };
