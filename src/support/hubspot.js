...
class HubSpotContract {
  ...
  prepare(fields, metadata, mapping) {
    ...
    const matchingStages = pipeline?.stages.filter(stage => normalizedLabel(stage.label) === normalizedLabel(fields.status)) || [];
    const stage = matchingStages.length === 1 ? matchingStages[0] : pipeline?.stages.find(s => s.id === mapping.stageId);
