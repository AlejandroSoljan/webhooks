...
async function startWorker() {
  ...
  const heartbeat = setInterval(async () => {
    ...
    try { await getDb(); await lease.renew(); } catch { await stop(); console.error('Support worker lost its lease; restart required.'); process.exitCode = 1; }
...
if (require.main === module) {
  startWorker().then(worker => {
    let closing = false;
    const shutdown = async () => { if (closing) return; closing = true; await worker.stop(); await closeDb(); process.exit(0); };
    process.on('SIGINT', shutdown); process.on('SIGTERM', shutdown); process.on('SIGBREAK', shutdown);
  }).catch(async () => { console.error('Support worker startup failed: check enable flag, keys, migration and worker lease.'); await closeDb(); process.exitCode = 1; });
}
