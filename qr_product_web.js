// Asisto | Version: 5.00.037 | Fecha: 2026-09-04
// qr_product_web.js
...
let PRODUCT=null, AI_ENABLED=false, sending=false, started=false, conversationId='', pollTimer=null, lastMessagesSignature='', scanStream=null, scanFrame=0;
...
function renderProduct(j){
  ...
  el('otherProductBtn').addEventListener('click',()=>{stopScanner();el('lookup').classList.remove('hidden');el('lookup').scrollIntoView({behavior:'smooth',block:'start'});el('codeInput').focus()});
}
function openCode(value){let code=String(value||'').trim();if(!code)return;try{const u=new URL(code,location.origin);code=u.searchParams.get('codigo')||((u.pathname.match(/^\/qr\/[^/]+\/(.+)$/)||[])[1])||code}catch(_){}try{code=decodeURIComponent(code).trim()}catch(_){}if(!code)return;stopScanner();location.href='/qr/'+encodeURIComponent(TENANT)+'?codigo='+encodeURIComponent(code)}
function stopScanner(){if(scanFrame)cancelAnimationFrame(scanFrame);scanFrame=0;if(scanStream){for(const track of scanStream.getTracks())track.stop();scanStream=null}el('scanner').classList.add('hidden')}
async function startScanner(){const status=el('scanStatus');el('scanner').classList.remove('hidden');if(!navigator.mediaDevices?.getUserMedia){status.textContent='La cámara no está disponible en este navegador. Ingresá el código manualmente.';return}if(!('BarcodeDetector' in window)){status.textContent='Este navegador no admite lectura automática. Ingresá el código manualmente.';return}try{const supported=await BarcodeDetector.getSupportedFormats();const wanted=['qr_code','ean_13','ean_8','upc_a','upc_e','code_128','code_39','itf','codabar'].filter(x=>supported.includes(x));const detector=new BarcodeDetector({formats:wanted});scanStream=await navigator.mediaDevices.getUserMedia({video:{facingMode:{ideal:'environment'}},audio:false});const video=el('scanVideo');video.srcObject=scanStream;await video.play();status.textContent='Apuntá al QR o al código de barras.';const detect=async()=>{if(!scanStream)return;try{const found=await detector.detect(video);if(found[0]?.rawValue){openCode(found[0].rawValue);return}}catch(_){}scanFrame=requestAnimationFrame(detect)};detect()}catch(_){stopScanner();el('scanner').classList.remove('hidden');status.textContent='No se pudo abrir la cámara. Podés ingresar el código manualmente.'}}
async function loadProduct(){try{el('productCard').innerHTML='<div class="loading">Consultando producto...</div>';const u=new URL('/api/ext/qr/product',location.origin);u.searchParams.set('tenant',TENANT);u.searchParams.set('codigo',CODE);renderProduct(await jsonFetch(u.toString()))}catch(e){el('productCard').innerHTML='<div class="error"><b>No encontramos este producto.</b><br/><span>'+esc(e.message)+'</span><div class="actions"><button class="btn btnPrimary" id="rescanProductBtn" type="button">Escanear o ingresar otro código</button><button class="btn" id="retryProductBtn" type="button">Reintentar este código</button></div></div>';const rb=el('retryProductBtn');if(rb)rb.addEventListener('click',loadProduct);const sb=el('rescanProductBtn');if(sb)sb.addEventListener('click',()=>{el('lookup').classList.remove('hidden');el('lookup').scrollIntoView({behavior:'smooth',block:'start'});el('codeInput').focus()})}}
