...
async function resolveOpenAiKey(db, tenant) {
  ...
  const channel = channels.find(c => c?.isDefault === true && String(c?.openaiApiKey || '').trim())
    ...
    || null;
  return resolveOpenAiApiKey('conversacional');
}
...
let PRODUCT=null, AI_ENABLED=false, sending=false, started=false, conversationId='', pollTimer=null, unchangedPolls=0, lastMessagesSignature='', scanStream=null, scanTrack=null, scanFrame=0, scanFallbackTimer=null, scanCandidate='', scanHits=0, scanCandidateAt=0, torchOn=false, scanOcrAttempted=false, scanOcrBusy=false;
...
function normalizedCode(value){let code=String(value||'').trim();if(!code)return '';try{const u=new URL(code,location.origin);if(/^https?:/i.test(code)){const extracted=u.searchParams.get('codigo')||((u.pathname.match(/^\/qr\/[^/]+\/(.+)$/)||[])[1]);if(!extracted)return '' ;code=extracted}}catch(_){}try{code=decodeURIComponent(code).trim()}catch(_){}return code} function openCode(value,via){const code=normalizedCode(value);if(!code)return;stopScanner();const u=new URL('/qr/'+encodeURIComponent(TENANT),location.origin);u.searchParams.set('codigo',code);u.searchParams.set('via',via||'sku');location.href=u.pathname+u.search} function openScannerPanel(){el('lookup').classList.remove('hidden');el('lookup').scrollIntoView({behavior:'smooth',block:'start'});startScanner()}
function stopScanner(hide=true){if(scanFrame)cancelAnimationFrame(scanFrame);scanFrame=0;if(scanFallbackTimer)clearInterval(scanFallbackTimer);scanFallbackTimer=null;scanTrack=null;torchOn=false;scanOcrBusy=false;if(scanStream){for(const track of scanStream.getTracks())track.stop();scanStream=null}el('scanBtn').classList.remove('hidden');el('zoomLabel').classList.add('hidden');el('torchBtn').classList.add('hidden');if(hide)el('scanner').classList.add('hidden')}
...
async function tryPrintedBarcode(retry=false){if((scanOcrAttempted&&!retry)||scanOcrBusy||!scanStream)return;const video=el('scanVideo');if(!video.videoWidth||!video.videoHeight)return;scanOcrAttempted=true;scanOcrBusy=true;const status=el('scanStatus'),c=document.createElement('canvas'),scale=Math.min(1,1280/Math.max(video.videoWidth,video.videoHeight));c.width=Math.round(video.videoWidth*scale);c.height=Math.round(video.videoHeight*scale);c.getContext('2d').drawImage(video,0,0,c.width,c.height);status.textContent='Leyendo el código… mantené el celular quieto.';try{const j=await jsonFetch('/api/ext/qr/photo',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({tenant:TENANT,image:c.toDataURL('image/jpeg',.86)})});const code=String(j.identification?.barcode||'').replace(/\D/g,'');if(/^\d{8,14}$/.test(code)){el('codeInput').value=code;stopScanner(false);video.classList.add('hidden');status.textContent='Número detectado: '+code+'. Buscando producto…';setTimeout(()=>openCode(code),250)}else if(scanStream)status.textContent=retry?'Alineá el código y mantené el celular quieto. Seguimos intentando…':'No pude leer el número. Acercá la cámara o ingresalo manualmente.'}catch(_){if(scanStream)status.textContent='Seguí enfocando el código o ingresá el número manualmente.'}finally{scanOcrBusy=false}}
...
function mountQrProductWeb(app) {
  ...
  app.get('/api/ext/qr/product', async (req, res) => {
    try {
      ...
      const via = String(req.query?.via || 'qr').toLowerCase();
      await recordMonetizationEvent({
        db,
        tenantId: tenant,
        eventKey: via === 'qr' ? 'catalog.qr_read' : 'catalog.code_lookup',
        sourceId: String(req.query?.eventId || `${via}:${code}:${Date.now()}`),
        metadata: { via, code }
      }).catch(error => console.warn('[monetization] qr event:', error?.message || error));
      res.setHeader('Cache-Control', 'no-store');
      return res.json({
        ok: true,
        build: QR_BUILD,
