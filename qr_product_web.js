...
<div class="page">
  ...
  <section class="card lookup" id="lookup"><div class="content"><div class="eyebrow">Buscar producto</div><h1 class="title">Escaneá o ingresá el código</h1><button class="btn btnPrimary" id="scanBtn" type="button">Abrir cámara</button><button class="btn photoAction" id="photoBtn" type="button">Buscar por foto</button><input class="hidden" id="photoInput" type="file" accept="image/*" capture="environment"/><div class="photoResult hidden" id="photoResult"></div><div class="lookupForm"><input id="codeInput" type="text" maxlength="180" autocomplete="off" placeholder="Código, SKU o código de barras"/><button class="btn" id="lookupBtn" type="button">Buscar</button></div><div class="scanner hidden" id="scanner"><div class="scannerViewport"><video id="scanVideo" playsinline muted></video><div class="scanGuide"></div></div><div class="scanStatus" id="scanStatus">Apuntá al QR o al código de barras.</div><div class="scanControls"><label id="zoomLabel" class="hidden">Zoom <input id="zoomControl" type="range" step="0.1"/></label><button class="btn hidden" id="torchBtn" type="button">Encender luz</button></div><button class="btn" id="stopScanBtn" type="button">Cerrar cámara</button></div></div></section>
...
el('photoBtn').addEventListener('click',()=>el('photoInput').click());el('photoInput').addEventListener('change',e=>identifyPhoto(e.currentTarget.files&&e.currentTarget.files[0]));
