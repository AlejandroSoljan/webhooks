...
// Helper: parseo tolerante de JSON para meta
function tryParseJSON(s){ try{ return JSON.parse(String(s||"")); } catch { return null; } }
...
const {
  ...
  // extras
buildSystemPrompt, bumpConversationTokenCounters, putInCache, resetSession, ensureMessageOnce, mergePedido} = require("./logic");
...
// Chat con historial (robusto a timeouts/errores)
  ...
  meta: { ai_json: json || null }   // ← el Pedido queda en el historial del API
            ...
            await sendSafeText(from, textToSend, value);
          // Merge incremental del Pedido en conversations.summary.Pedido
          try {
            const ai = json || tryParseJSON(content);
            if (ai && ai.Pedido) {
              const db = await getDb();
              const cur = await db.collection("conversations").findOne({ _id: conv._id });
              const prev = cur?.summary?.Pedido || {};
              const merged = mergePedido(prev, ai.Pedido);
              await db.collection("conversations").updateOne(
                { _id: conv._id },
                { $set: { "summary.Pedido": merged, updatedAt: new Date() } }
              );
            }
          } catch (e) {
            console.warn("merge Pedido warn:", e?.message || e);
          }
            // Finalización (COMPLETED/CANCELLED)
            const estado = json?.estado;
            if (estado === "COMPLETED" || estado === "CANCELLED") {
